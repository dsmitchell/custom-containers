...
class CanvasView: UIView {
	...
	public var currentWidth: CGFloat = 3
    ...
    */
    ...
    override func draw(_ rect: CGRect) {
        ...
        if needsFullRedraw {
			...
			// Draw "eraser" lines on the frozen context. All other lines will be on the current context
			for line in lines.filter({ UIColor.clear.isEqual($0.color) }) {
		...
		// We cannot draw "eraser" lines on the current context
        for line in lines.filter({ !UIColor.clear.isEqual($0.color) }) {
            line.drawInContext(context: context, isDebuggingEnabled: isDebuggingEnabled, usePreciseLocation: usePreciseLocations)
        }
    }
    ...
    func addActiveLineForTouch(touch: UITouch) -> Line {
		let newLine = Line(color: currentColor, width: currentWidth)
